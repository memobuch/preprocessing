    ...
    def load_data(self):
        ...
        for person_entry in persons_dict:
            ...
            if not os.path.exists(person_folder_path):
                ...
                self.logger.warning(f"Skipping Gdrive data (images, documents,fluchtorte, haftorte) for person {cur_memo_person.id}. ")
                continue
    ...
    def _load_metadata_csv(self, path: str):
        ...
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as txt_file:
                csv_reader = csv.DictReader(txt_file, lineterminator='\n', delimiter=',', quotechar="'")
